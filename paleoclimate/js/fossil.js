...
var bannerHeight = 30;
var filterTaxa, filterGenus, globalData, firstTime=true, color, taxa, genusSelections, colorCode = "taxa", colorLookup = {"taxa": 'filterTaxa', "genus": 'filterGenus'}
...
function initialize(){
  ...
  //load data.
  d3.csv("data/fossils.csv", function(data) {
    ...
    Taxa2Genus = d3.nest() //rollup unique taxa by genus
      .key(function(d) { return d.taxa})
      .key(function(d) {return d.genus})
      .entries(data);
    taxa = []
    for (p=0; p<Taxa2Genus.length;p++){
      x = d3.values(Taxa2Genus[p])[1].map(function(d){return d.key}) //this function gets genus names for specified taxa
      e = Taxa2Genus[p].key
      taxa[e] = d3.values(x)
    }
    ...
    filterGenus = data.map(function(d){return d.genus}) //array of genus from data
    filterGenus = d3.set(filterGenus).values().sort()//need to eliminate redundancies

    color = d3.scale.category20()
      .domain(filterTaxa)
      //.range(['#edf8e9', '#bdd7e7','#6baed6','#3182bd','#08519c']); //inappropriate color scheme

    var max = d3.max(data, function(d) {return d.pollen});
...
function filterSelectorCreator(){
  ...
  d3.select("body") //or svg?
    ...
    .style({"width": filterWidth, "height": filterHeight, "top": height100/10+"px", "right": "30px"})
}
...
/*  tooltip = d3.tip()
  ...
  .offset([-5, 0])
  .html(function(d) {
    return "<span style='color:white'>" + d.site + "</span>";
  })
  svg.call(tooltip)*/

  data.sort(function(a,b){return b.pollen - a.pollen}) //sort data so 'smaller' sites are drawn last

  var sites = svg.append("g")
  sites.selectAll("circle")
    .data(data)
    .enter().append("circle")
    .attr("class", function(d) {return d.genus+" "+d.taxa+" "+d.site+" "+d.year})
    //.attr("id", "")
    .style("fill", function(d){return color(d[colorCode])})
    .style(defaultStroke)
    .attr("cx", function(d) {return projection([d.long, d.lat])[0]; }) 
    .attr("cy", function(d) { return projection([d.long, d.lat])[1]; })
    .attr("r", function(d) { return radiusFlannery(d.pollen); })
...
function setControls(data){
  ...
  //color switcher
  var form = d3.select(".filterSelector").append("form"), j=0;//use j to make default taxa?
  form.append("text")
  ...
  labels.append('input')
    ...
    .on("click", function(d){
      colorCode = d
      color.domain(colorLookup[colorCode])
  ...
  var labels = form.append("select")
    ...
    .on("change", function(){
      ...
      //selections = selections.map(function(d){return filterTaxa[d]})

      //reload
      reload(selections, "taxa") //reload everything selected
    })
    .selectAll("option")
    .data(filterTaxa)
    .enter().append("option")
    ...
    genusSelections=filterGenus
  }

  //genus here
  //for changing genus selection based on taxa selection
  taxaSelections=[]
  taxaform = d3.select("#filterTaxaSelect")[0][0]
  for (l=0; l<taxaform.length;l++){if (taxaform[l].selected){taxaSelections.push(taxaform[l].value)}}

  d3.select(".genusBox").remove()
  var form = d3.select(".filterSelector").append("div").attr("class", "genusBox").append("form"), j=0;
  form.append("text")
    .attr("class", "viewerCategory")
    .text("Select Genus:")
  var labels = form.append("select")
      .attr({
        id: "filterGenusSelect",
        multiple: true,
        size: 10
      })
      .on("change", function(){
        ...
        var selections = []
        for (x=0;x<yyy.length;x++){if (yyy[x].selected){selections.push(yyy[x].value)}}
        genusSelections=selections

        //change selection
        reload(selections, "genus", taxaSelections)
      })
      .selectAll("option")
      .data(filterGenus)
      .enter().append("option")
      .attr({
          type: "option",
          selected: function(d){
            if (genusSelections.includes(d)){return true}
          },
          value: function(d, i) {return d;}
      })
      .text(function(d) {return d;})

  //filter years selector. brusher
  var filterYear = data.map(function(d){return d.year})
  var xxxx = d3.nest()
    .key(function(d) {return d})
    .sortKeys(d3.descending())
    .map(filterYear);
  n=d3.keys(xxxx).map(function(d){return Number(d)})
  n.sort(function(a,b){return b-a})
  p=[]
  for (z=0;z<n.length; z++){p[z]={'year':n[z], 'count': xxxx[n[z]].length}}
  
 // filterYear = d3.set(filterYear).values()//need to eliminate redundancies for purposes of legend, but need count
  svg.selectAll("rect, text").remove()
   
  var w = width100/5
  var h = height100/5

  var max = d3.max(p, function(d){return d.count}), min = d3.min(p, function(d){return d.count})
  var maxx = d3.max(p, function(d){return d.year}), minx=d3.min(p, function(d){return d.year})
  var xplace = width100/1.33
  var yplace = height100/1.5
  var y = d3.scale.log()
    .domain([min,max])
    .range([1,h]);
  var x = d3.scale.log()
  ...
  var t= svg.append('g').attr("width", w).attr('transform', "translate("+xplace+","+yplace+")")
  t.selectAll("rect")
      .data(p)
      .enter().append("rect")
      .attr("x", function(d, i) {return i*barw})
      .attr("y", function(d) { return h-y(d.count) })
      .attr("width", barw) //scale years?
  ...
  //implement brush here
  function brushy (){
    var extent = brush.extent()
    console.log(extent)
  }
  legend(taxaSelections)
}

function legend(taxaSelections){
  console.log(taxaSelections)
  var l1 = svg.append('g').attr('class', 'legend').attr('transform', "translate("+width100/1.2+","+height100/10+")")
  l1.selectAll("rect")
      .data(taxaSelections)
      .enter().append("rect")
      .attr("x", 0)
      .attr("y", function(d,i) { return i*10+15 })
      .attr("width", 10) //scale years?
      .attr("height", 10)
      .style("fill", function(d){if (colorCode =="taxa"){return color(d)}})
  l1.selectAll("text")
      .data(taxaSelections)
      .enter()
      .append("text")
      .text(function(d){return d})
      .attr("x", 15)
      .attr("y", function(d,i){return i*10.5+21})

  var l2 = svg.append('g').attr('class', 'legend').attr('transform', "translate("+width100/1.2+","+height100/4+")")
  l2.selectAll("rect")
      .data(filterGenus)
      .enter().append("rect")
      .attr("x", 0)
      ...
      .attr("width", 10) //scale years?
      .attr("height", 10)
      .style("fill", function(d){if (colorCode =="genus"){return color(d)}})
  l2.selectAll("text")
      .data(filterGenus)
      .enter()
      .append("text")
...
//reload data
function reload (selections, kind, taxaSelections){
  firstTime = false
  ...
  if (kind == "taxa"){
    var reload = globalData.filter(function(d){return selections.includes(d[kind])})
    filterGenus = reload.map(function(d){return d.genus}) //array of genus from data
    filterGenus = d3.set(filterGenus).values().sort()//need to eliminate redundancies
    genusSelections = filterGenus
  }
  if (kind == "genus"){
    //look taxa selections up in Taxa2Genus.filter globaldata by taxa then by genus
    var reload = globalData.filter(function(d){return taxaSelections.includes(d.taxa)})
    reload = reload.filter(function(d){return selections.includes(d.genus)})
  }
